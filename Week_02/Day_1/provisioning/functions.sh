...
function create-key-pair(){
    ...
    if [ ! -e ${INSTANCE_DIR}/${SECURITY_GROUP_NAME}.pem ]; then
        aws ec2 create-key-pair --key-name ${SECURITY_GROUP_NAME} --query 'KeyMaterial' --output text > ./ec2_instance/${SECURITY_GROUP_NAME}.pem
        chmod 400 ./ec2_instance/${SECURITY_GROUP_NAME}.pem
    fi
...
function delete-security-group(){
    ...
    if [ -e ./ec2_instance/security-group-id.txt ]; then
        ...
        rm -f ./ec2_instance/${SECURITY_GROUP_NAME}.pem
    fi
}
